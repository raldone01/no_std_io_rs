...
pub struct CursorMut<'a, B: ?Sized> {
  backing_buffer: &'a mut B,
  position: usize,
}

impl<'a, B: ?Sized> CursorMut<'a, B> {
  #[must_use]
  pub fn new(backing_buffer: &'a mut B) -> Self {
    Self {
      backing_buffer,
      position: 0,
    }
...
impl<B: AsRef<[u8]> + ?Sized> CursorMut<'_, B> {
  #[must_use]
...
impl<B: BackingBufferMut + ?Sized> CursorMut<'_, B> {
  #[must_use]
...
impl<B: AsRef<[u8]>> Seek for CursorMut<'_, B> {
...
impl<B: AsRef<[u8]> + ?Sized> Read for CursorMut<'_, B> {
  type ReadError = Infallible;

  fn read(&mut self, output_buffer: &mut [u8]) -> Result<usize, Self::ReadError> {
    let n = Read::read(&mut CursorMut::split(self).1, output_buffer)?;
    self.position += n;
    Ok(n)
  }
}
...
impl<B: AsRef<[u8]> + ?Sized> BufferedRead for CursorMut<'_, B> {
...
impl<B: BackingBufferMut + ?Sized> BackingBufferMut for CursorMut<'_, B> {
...
impl<B: AsRef<[u8]> + ?Sized> AsRef<[u8]> for CursorMut<'_, B> {
  fn as_ref(&self) -> &[u8] {
    self.backing_buffer.as_ref()
  }
}

impl<B: AsMut<[u8]> + ?Sized> AsMut<[u8]> for CursorMut<'_, B> {
  fn as_mut(&mut self) -> &mut [u8] {
    self.backing_buffer.as_mut()
  }
}

impl<B: BackingBufferMut + ?Sized> Write for CursorMut<'_, B> {
